<script>
	...
	const raw_points = y_values.map((y, index) => {
		return { x: index, y: y };
	});
	...
	async function initChart() {
		...
		chart = new ApexCharts(chartEl, {
			...
			series: final_points.map((points) => {
				return { name: labelName, data: points };
			})
		});
		chart.render();
	}
...
<div bind:this={chartEl} class="h-[350px] w-full"></div>
