<script lang="ts">
	...
	let generated_last_60_to_last_30_percent = $derived.by(() => {
		return (
			((generated_last_30 - generated_last_60_to_last_30) /
				Math.abs(generated_last_60_to_last_30)) *
			100
		);
	});
	...
	let consumed_last_60_to_last_30_percent = $derived.by(() => {
		return (
			((consumed_last_30 - consumed_last_60_to_last_30) / Math.abs(consumed_last_60_to_last_30)) *
			100
		);
	});
</script>
